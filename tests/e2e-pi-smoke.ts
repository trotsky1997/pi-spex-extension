...
import {
	...
	getShipResumeDecision,
} from "../extensions/spex/ship.js";
import {
	createShipState,
	failShipState,
	pauseShipState,
...
const ASK_USER_PKG_DIR = resolve(
	ROOT_DIR,
	"node_modules",
					...
					questions: [
						{
							...
							options: [
								...
								{
									label: "deep-review",
									description: "Enable deep review guidance.",
								},
								{
									label: "worktrees",
									description: "Enable worktree guidance.",
								},
							],
						},
						{
...
function buildShipResponses() {
	return [
		...
		fauxAssistantMessage([
			fauxText("Advanced the spex ship pipeline by one stage."),
		]),
	];
}
...
function buildShipResumeResponses() {
	return [
		...
		fauxAssistantMessage([
			fauxText("Resumed the paused spex ship pipeline and advanced it."),
		]),
	];
}
...
async function main(): Promise<void> {
	...
	try {
		...
		await execFile("pi", ["install", "-l", ASK_USER_PKG_DIR], { cwd: repo });
		...
		assert(
			...
			`resource loader reported extension errors: ${JSON.stringify(extensions.errors)}`,
		);
		...
		assert(
			sessionResult.extensionsResult.errors.length === 0,
			`session extension load errors: ${JSON.stringify(sessionResult.extensionsResult.errors)}`,
		);
		const toolNames = sessionResult.session.agent.state.tools.map(
			(tool) => tool.name,
		);
		assert(
			toolNames.includes("AskUserQuestion"),
			`expected AskUserQuestion tool, got: ${toolNames.join(", ")}`,
		);
		assert(
			toolNames.includes("spex_init_project"),
			`expected spex_init_project tool, got: ${toolNames.join(", ")}`,
		);
		assert(
			getCommand(sessionResult, "spex-init"),
			"expected spex-init command to be registered",
		);
		assert(
			getCommand(sessionResult, "spex-ship"),
			"expected spex-ship command to be registered",
		);
		...
		assert(
			config.traits.superpowers === true,
			"expected superpowers trait to be enabled after /spex-init",
		);
		assert(
			config.traits["deep-review"] === true,
			"expected deep-review trait to be enabled after /spex-init",
		);
		assert(
			config.permissions === "standard",
			"expected standard permissions after /spex-init",
		);

		const speckitPrompt = await readFile(
			join(repo, ".pi", "prompts", "speckit.specify.md"),
			"utf8",
		);
		assert(
			speckitPrompt.toLowerCase().includes("feature specification"),
			"expected Spec Kit Pi prompt to be generated",
		);
		...
		assert(
			await waitForFile(shipStatePath, 5000),
			"spex-ship command did not create a ship state file",
		);
		...
		assert(
			shipState.stage === "review-code",
			`expected ship stage to advance to review-code, got ${shipState.stage}`,
		);
		assert(
			shipState.stageIndex === 7,
			`expected ship stageIndex 7, got ${shipState.stageIndex}`,
		);
		assert(
			shipState.status === "running",
			`expected ship status running, got ${shipState.status}`,
		);
		...
		assert(
			...
			"expected ship state to become paused",
		);
		...
		assert(
			shipState.status === "running",
			`expected paused resume to switch status back to running, got ${shipState.status}`,
		);
		if (shipState.stage === "review-code" && paused.state) {
			const decision = getShipResumeDecision(paused.state);
			assert(
				decision.kind === "resume",
				"paused ship should produce a resume decision",
			);
		...
		assert(
			shipState.stage === "stamp",
			`expected paused resume to advance to stamp, got ${shipState.stage}`,
		);
		assert(
			shipState.stageIndex === 8,
			`expected paused resume stageIndex 8, got ${shipState.stageIndex}`,
		);
		assert(
			shipState.status === "running",
			`expected resumed ship status running, got ${shipState.status}`,
		);
		...
		await waitForAgent(sessionResult.session);
		assert(
			!existsSync(shipStatePath),
			"expected ship state file to be removed after completing stamp",
		);
		...
		assert(
			recreated.state?.stage === "stamp",
			"expected recreated ship state to start at stamp",
		);
		...
		assert(
			...
			"expected ship state to become failed",
		);
		...
		assert(
			shipState.stage === "stamp",
			`expected failed resume to keep stamp stage, got ${shipState.stage}`,
		);
		assert(
			shipState.status === "failed",
			`expected failed resume to leave status failed, got ${shipState.status}`,
		);
