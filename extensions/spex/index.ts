...
const BUNDLED_ASK_USER_EXTENSION_PATH = join(
	PACKAGE_ROOT,
	"node_modules",
	"pi-claude-code-ask-user",
	"extensions",
...
let bundledAskUserLoadAttempted = false;
...
function updateStatus(
	...
	ctx.ui.setStatus(
		"pi-spex",
		enabled === "none" ? "spex:ready" : `spex:${enabled}`,
	);
}
	...
	return {
		...
		traits: options.traits
			? {
					...current.traits,
					...(Object.fromEntries(
						SUPPORTED_TRAITS.map((trait) => [trait, false]),
					) as Record<(typeof SUPPORTED_TRAITS)[number], boolean>),
					...(Object.fromEntries(
						parseTraitCsv(options.traits.join(",")).map((trait) => [
							trait,
							true,
						]),
					) as Record<(typeof SUPPORTED_TRAITS)[number], boolean>),
...
function hasAskUserQuestionTool(pi: ExtensionAPI): boolean {
	return pi
		.getAllTools()
		.some(
			(tool: { name: string }) => tool.name === ASK_USER_QUESTION_TOOL_NAME,
		);
}

async function ensureAskUserQuestionTool(pi: ExtensionAPI): Promise<boolean> {
	if (hasAskUserQuestionTool(pi)) {
		return true;
	}

	if (bundledAskUserLoadAttempted) {
		return false;
	}

	bundledAskUserLoadAttempted = true;

	try {
		const module = await import(
			pathToFileURL(BUNDLED_ASK_USER_EXTENSION_PATH).href
		);
		const register = (
			module as { default?: ((api: ExtensionAPI) => void) | undefined }
		).default;
	...
	return hasAskUserQuestionTool(pi);
}

function normalizeTraits(
	input: string[] | undefined,
): (typeof SUPPORTED_TRAITS)[number][] {
	return (input ?? []).filter(
		(trait): trait is (typeof SUPPORTED_TRAITS)[number] =>
...
async function executeUpdateTraits(
	...
	const disable = normalizeTraits(params.disable).filter(
		(trait) => !enable.includes(trait),
	);
	const ignoredDisable = normalizeTraits(params.disable).filter((trait) =>
		enable.includes(trait),
	);
	...
	const changedPermissions =
		params.permissions &&
		SUPPORTED_PERMISSION_LEVELS.includes(params.permissions)
			? params.permissions
			: undefined;
...
async function handleInit(
	...
	if (!(await ensureAskUserQuestionTool(pi))) {
		if (parsed.permissions && parsed.traitCsv !== undefined) {
			const result = await executeInitProject(
				pi,
				{
					refresh: parsed.refresh,
					update: parsed.update,
					traits: parseTraitCsv(parsed.traitCsv),
					permissions: parsed.permissions,
				},
				ctx,
			);
			await openMarkdown(ctx, result.title, result.body);
			return;
		}

		ctx.ui.notify(
			"`AskUserQuestion` is not available. Install `pi-claude-code-ask-user`, or rerun `/spex-init --traits <csv> --permissions <none|standard|yolo>`.",
			"warning",
		);
		return;
	}

	const preselectedTraits = parsed.traitCsv
		? parseTraitCsv(parsed.traitCsv)
		: [];
	...
	ctx.ui.notify(
		"Queued spex init workflow. The agent will use AskUserQuestion for trait and permission selection.",
		"info",
	);
}
...
async function handleTraits(
	...
	if (action === "list" || action === "show") {
		await openMarkdown(
			ctx,
			"pi-spex traits",
			getTraitsSummary(config, getConfigPath(ctx.cwd)),
		);
		return;
	}
	...
	if (action === "enable") {
		...
		if (traits.length === 0) {
			ctx.ui.notify(
				`No valid trait provided. Supported: ${SUPPORTED_TRAITS.join(", ")}`,
				"warning",
			);
			return;
		}
	...
	if (action === "permissions") {
		...
		if (!SUPPORTED_PERMISSION_LEVELS.includes(level)) {
			ctx.ui.notify(
				`Invalid permission level. Use one of: ${SUPPORTED_PERMISSION_LEVELS.join(", ")}`,
				"warning",
			);
			return;
		}
	...
	if (action === "interactive" || action === "ask" || action === "configure") {
		if (!(await ensureAskUserQuestionTool(pi))) {
			ctx.ui.notify(
				"`AskUserQuestion` is not available. Install `pi-claude-code-ask-user`, then run `/spex-traits interactive` again.",
				"warning",
			);
			return;
		}
		...
		ctx.ui.notify(
			"Queued interactive spex-traits workflow. The agent will use AskUserQuestion before updating config.",
			"info",
		);
		return;
	}
	...
	ctx.ui.notify(
		"Usage: /spex-traits [list|enable <csv>|disable <csv>|permissions <none|standard|yolo>|interactive]",
		"warning",
	);
}
...
async function handleWorktree(
	...
	if (action === "list" || !action) {
		...
		if (result.code !== 0) {
			throw new Error(
				(result.stderr || result.stdout || "git worktree list failed").trim(),
			);
		}
		await openMarkdown(
			ctx,
	...
	if (action === "prune") {
		const result = await pi.exec("git", ["worktree", "prune"], {
			cwd: ctx.cwd,
			timeout: 15000,
		});
		if (result.code !== 0) {
			throw new Error(
				(result.stderr || result.stdout || "git worktree prune failed").trim(),
			);
		}
	...
	if (action === "add") {
		...
		ctx.ui.notify(
			(result.stdout || `Created worktree at ${pathArg}`).trim(),
			"info",
		);
		return;
	}
...
export default function spex(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		description:
			"Bootstrap Spec Kit for Pi and persist pi-spex trait configuration after the user answers AskUserQuestion prompts.",
		promptSnippet:
			"Bootstrap Spec Kit for Pi after collecting trait and permission answers.",
		promptGuidelines: [
		...
		parameters: Type.Object({
			refresh: Type.Optional(
				Type.Boolean({
					description: "Force a refresh bootstrap of the current project.",
				}),
			),
			update: Type.Optional(
				Type.Boolean({
					description: "Upgrade the external specify CLI before bootstrapping.",
				}),
			),
			traits: Type.Optional(
				Type.Array(Type.String({ description: "Trait name." }), {
		...
		async execute(
			...
			const permissions = params.permissions?.trim() as
				| PermissionLevel
				| undefined;
			const result = await executeInitProject(
				pi,
				{
					...
					traits: (params.traits ?? []).filter(
						(trait): trait is (typeof SUPPORTED_TRAITS)[number] =>
							SUPPORTED_TRAITS.includes(
								trait as (typeof SUPPORTED_TRAITS)[number],
							),
					),
					permissions:
						permissions && SUPPORTED_PERMISSION_LEVELS.includes(permissions)
							? permissions
							: undefined,
				},
				ctx,
	...
	pi.registerTool({
		...
		description:
			"Persist pi-spex trait and permission updates after the user answers AskUserQuestion prompts.",
		promptSnippet:
			"Update pi-spex trait state after AskUserQuestion collects the requested changes.",
		promptGuidelines: [
		...
		parameters: Type.Object({
			enable: Type.Optional(
				Type.Array(Type.String({ description: "Trait name to enable." }), {
					description: "Traits to enable.",
				}),
			),
			disable: Type.Optional(
				Type.Array(Type.String({ description: "Trait name to disable." }), {
	...
	pi.registerCommand("spex-worktree", {
		description:
			"List, add, or prune git worktrees for spex-style feature isolation",
		handler: async (args: string, ctx: ExtensionCommandContext) =>
			handleWorktree(pi, args, ctx),
	});
	...
	pi.on("context", async (event: any) => {
		const lastGenericIndex = event.messages.reduce(
			(acc: number, message: any, index: number) => {
	...
	pi.on("session_start", async (_event: unknown, ctx: ExtensionContext) => {
		await ensureAskUserQuestionTool(pi);
